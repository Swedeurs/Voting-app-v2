...
export function createRepository(db: Db) {
    ...
    async getElectionById(id: number) {
      const [election] = await db.select().from(electionTable).where(eq(electionTable.id, id));
      return election;
    },
    async updateElection(id: number, updates: ElectionUpdates) {
      return await db.update(electionTable).set(updates).where(eq(electionTable.id, id)).returning();
    },
