...
export function ElectionDetail({
  ...
  const [votedForRepresentative, setVotedForRepresentative] = useState<
    number | null
  >(null);
  const [votedForAlternative, setVotedForAlternative] = useState<string | null>(
    null,
  );
        ...
        {representatives.map((rep) => (
          <div key={rep.id} className="border p-4 rounded shadow bg-white">
            ...
            <p>
              ...
              {votes.representatives[rep.id] || 0}
            </p>
            {!isConcluded && (
              <button
                onClick={() => handleRepresentativeVote(rep.id)}
                className={`bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 mt-2 ${
                  votedForRepresentative !== null
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {alternatives.map((alt: string, index: number) => (
          <div key={index} className="border p-4 rounded shadow bg-white">
            ...
            <p>
              <span className="font-semibold">Votes:</span>{" "}
              {votes.alternatives[alt] || 0}
            </p>
            {!isConcluded && (
              <button
                onClick={() => handleAlternativeVote(alt)}
                className={`bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 mt-2 ${
                  votedForAlternative !== null
                    ? "opacity-50 cursor-not-allowed"
                    : ""
                }`}
